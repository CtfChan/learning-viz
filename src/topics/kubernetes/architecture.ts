...
export const NODE_POSITIONS: Record<string, Position> = {
  ...
  "kube-apiserver": { x: 541, y: 155 },
  etcd: { x: 64, y: 203 },
  "kube-scheduler": { x: 63, y: 101 },
  "kube-controller-manager": { x: 141, y: 291 },
  "cloud-controller-manager": { x: 534, y: 290 },
  // Worker Node components
  kubelet: { x: 94, y: 253 },
  "kube-proxy": { x: 100, y: 119 },
  "container-runtime": { x: 385, y: 253 },
  // External components
  "cloud-provider-api": { x: 534, y: 480 },
};
...
export const ARCHITECTURE_CONFIG: ArchitectureConfig = {
  groups: [
    ...
    },
    {
      ...
      name: "Worker Node",
      description:
        "Runs containerized applications and provides the Kubernetes runtime environment",
      color: "#2196F3",
    },
  ],
  components: [
    ...
    },
    {
      ...
      group: "control-plane",
      description:
        "Embeds cloud-specific control logic. Only runs on cloud provider environments.",
      responsibilities: [
    ...
    // External components (no group)
    {
      id: "cloud-provider-api",
      name: "Cloud Provider API",
      shortName: "Cloud API",
      description:
        "External cloud provider APIs (AWS, GCP, Azure, etc.) that the cloud-controller-manager communicates with.",
      responsibilities: [
        "Manages cloud-specific resources",
        "Provisions load balancers",
        "Manages node lifecycle in cloud",
        "Configures cloud networking/routes",
      ],
      docsUrl:
  ...
  connections: [
    ...
    {
      from: "cloud-controller-manager",
      to: "cloud-provider-api",
      label: "manages cloud resources",
      sourceHandle: "bottom",
      targetHandle: "top",
    },
  ],
